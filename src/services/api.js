...
const apiClient = {
  ...
  // Métodos para modelos de IA
  
  // Obter todos os modelos de IA disponíveis
  getAIModels: async () => {
    try {
      const response = await api.get('/v1/ai/models');
      return response.data;
    } catch (error) {
      console.error('Erro ao obter modelos de IA:', error);
      throw error;
    }
  },
  
  // Obter detalhes de um modelo específico
  getAIModel: async (modelId) => {
    try {
      const response = await api.get(`/v1/ai/models/${modelId}`);
      return response.data;
    } catch (error) {
      console.error(`Erro ao obter detalhes do modelo ${modelId}:`, error);
      throw error;
    }
  },
  
  // Obter configurações de IA para uma câmera
  getCameraAISettings: async (cameraId) => {
    try {
      const response = await api.get(`/v1/ai/cameras/${cameraId}/settings`);
      return response.data;
    } catch (error) {
      console.error(`Erro ao obter configurações de IA para câmera ${cameraId}:`, error);
      throw error;
    }
  },
  
  // Atualizar configurações de IA para uma câmera
  updateCameraAISettings: async (cameraId, settings) => {
    try {
      const response = await api.put(`/v1/ai/cameras/${cameraId}/settings`, settings);
      return response.data;
    } catch (error) {
      console.error(`Erro ao atualizar configurações de IA para câmera ${cameraId}:`, error);
      throw error;
    }
  }
};
