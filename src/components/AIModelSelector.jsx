...
const AIModelSelector = ({ cameraId, onSave }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const settings = await apiClient.getCameraAISettings(cameraId);
        ...
        } else if (modelsResponse && Array.isArray(modelsResponse) && modelsResponse.length > 0) {
          // Selecionar o primeiro modelo como padrão
          setSelectedModel(modelsResponse[0].id);
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      await apiClient.updateCameraAISettings(cameraId, currentSettings);
