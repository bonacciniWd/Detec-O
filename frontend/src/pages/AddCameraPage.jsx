...
const AddCameraPage = () => {
  ...
  const discoverCameras = async () => {
    ...
    try {
      // Chamar a API real de descoberta de câmeras usando o método específico
      const response = await apiClient.discoverCameras({ 
        discovery_methods: ["auto"],
        timeout: 10.0
      });
      
      // Mapear os dados retornados para o formato esperado pelo componente
      const formattedCameras = response.map(camera => ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Chamar a API para conectar e adicionar a câmera usando o método específico
      const response = await apiClient.connectCamera(deviceData);
